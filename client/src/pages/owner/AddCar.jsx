...
const AddCar = () => {
 const { axios, currency } = useAppContext()
  ...
  const onSubmitHandler = async (e) => {
        ...
        if (isLoading) {
            // toast.error("Can't add 2 cars at same time")
            return null;
        }
   ...
   <div className='px-4 py-10 md:px-10 flex-1'>
            ...
            <form className='flex flex-col gap-5 text-gray-500 text-sm mt-6 max-w-xl' onSubmit={onSubmitHandler}>
                ...
                <div class="flex flex-col w-full">
                    ...
                    <textarea rows="5" placeholder="e.g. A luxurious SUV with a spacious interior and a powerful engine." required class="px-3 py-2 mt-1 border border-borderColor rounded-md outline-none" value={car.description} onChange={e => setCar({ ...car, description: e.target.value })}></textarea>
                </div>

                <button className='flex items-center gap-2 px-4 py-2.5 mt-4 bg-primary text-white rounded-md font-medium w-max cursor-pointer' disabled={isLoading}>
                    <img src={assets.tick_icon} alt="tick_icon" /> {isLoading ? 'Listing...' : 'List your car'}
                </button>
            </form>
        </div>
  )
